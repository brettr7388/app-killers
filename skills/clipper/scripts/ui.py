#!/usr/bin/env python3
"""Vertical clip cutter — local control panel.

    python3 ui.py

Opens http://localhost:7301 in your browser. Everything runs on this machine;
...
PORT = int(os.environ.get("PORT", "7301"))
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "output")
os.makedirs(OUT, exist_ok=True)

ACTIONS = [{'id': 'clip', 'label': 'Cut a vertical clip'}]
FIELDS = [{'n': 'file', 'l': 'Video file', 't': 'pick', 'k': 'file', 'p': 'the source video'}, [{'n': 'start', 'l': 'Start', 'v': '0:00'}, {'n': 'end', 'l': 'End', 'v': '0:20'}], [{'n': 'style', 'l': 'Captions', 't': 'select', 'o': [['opus', 'Opus — white + gold'], ['punch', 'Punch — ALL CAPS'], ['clean', 'Clean — no highlight']]}, {'n': 'captions', 'l': 'Burn captions', 't': 'select', 'o': [['on', 'Yes'], ['off', 'No']]}]]
...
PAGE = """<!doctype html><html><head><meta charset="utf-8"><title>Vertical clip cutter</title>
...
<header><h1>Clipper <b>panel</b></h1>
<span>localhost:7301 &middot; runs on this machine &middot; nothing uploaded</span></header>
...
def build_command(b):
    tool = os.path.join(HERE, "make_clip.py")
    f = b.get("file", "").strip()
    if not f or not os.path.exists(f):
        return None, f"File not found: {f or '(empty)'}"
    cmd = [sys.executable, tool, f, b.get("start", "0"), b.get("end", "10"),
           os.path.join(OUT, "clip.mp4"), "--style", b.get("style", "opus")]
    if b.get("captions") != "on":
