#!/usr/bin/env python3
"""PDF toolkit — local control panel.

    python3 ui.py

Opens http://localhost:7311 in your browser. Everything runs on this machine;
...
PORT = int(os.environ.get("PORT", "7311"))
HERE = os.path.dirname(os.path.abspath(__file__))
OUT = os.path.join(HERE, "output")
os.makedirs(OUT, exist_ok=True)

ACTIONS = [{'id': 'info', 'label': 'Info — pages, size, text layer'}, {'id': 'merge', 'label': 'Merge two files'}, {'id': 'split', 'label': 'Split — keep a page range'}, {'id': 'rotate', 'label': 'Rotate'}, {'id': 'text', 'label': 'Extract text'}, {'id': 'compress', 'label': 'Compress'}]
FIELDS = [{'n': 'file', 'l': 'PDF file', 't': 'pick', 'k': 'file', 'p': 'drag a path in, or hit Choose'}, {'n': 'file2', 'l': 'Second PDF (merge only)', 't': 'pick', 'k': 'file', 'p': 'only needed for merge'}, [{'n': 'pages', 'l': 'Pages (split)', 'v': '1-2'}, {'n': 'deg', 'l': 'Degrees (rotate)', 'v': '90'}]]
...
PAGE = """<!doctype html><html><head><meta charset="utf-8"><title>PDF toolkit</title>
...
<header><h1>PDF <b>panel</b></h1>
<span>localhost:7311 &middot; runs on this machine &middot; nothing uploaded</span></header>
...
def build_command(b):
    tool = os.path.join(HERE, "pdftool.py")
    a, f, f2 = b.get("action"), b.get("file", "").strip(), b.get("file2", "").strip()
    if not f or not os.path.exists(f):
        return None, f"File not found: {f or '(empty)'}"
    base = [sys.executable, tool, a, f]
    if a == "merge":
        if not f2 or not os.path.exists(f2):
            return None, "Merge needs a second PDF."
        return base + [f2, "-o", os.path.join(OUT, "merged.pdf")], None
    if a == "split":
        return base + ["--pages", b.get("pages", "1"),
                       "-o", os.path.join(OUT, "split.pdf")], None
    if a == "rotate":
        return base + ["--deg", b.get("deg", "90"),
                       "-o", os.path.join(OUT, "rotated.pdf")], None
    if a == "compress":
        return base + ["-o", os.path.join(OUT, "compressed.pdf")], None
    if a == "text":
        return base + ["-o", os.path.join(OUT, "extracted.txt")], None
    return base, None
