#!/usr/bin/env python3
"""Local transcription — local control panel.

    python3 ui.py

Opens http://localhost:7302 in your browser. Everything runs on this machine;
...
ACTIONS = [{'id': 'transcribe', 'label': 'Transcribe to a document'}]
FIELDS = [{'n': 'file', 'l': 'Audio or video file', 't': 'pick', 'k': 'file', 'p': 'mp3, m4a, wav, mp4, mov…'}, [{'n': 'model', 'l': 'Model', 't': 'select', 'o': [['tiny.en', 'tiny.en — fastest'], ['base.en', 'base.en — better'], ['small.en', 'small.en — best, slow']]}, {'n': 'speakers', 'l': 'Guess speakers', 't': 'select', 'o': [['no', 'No'], ['yes', 'Yes']]}]]
...
PAGE = """<!doctype html><html><head><meta charset="utf-8"><title>Local transcription</title>
<meta name="viewport" content="width=device-width,initial-scale=1"><style>
:root{--bg:#0d1015;--card:#161b22;--line:#242c38;--fg:#e6edf3;--dim:#8b97a6;--acc:#7dd87d}
...
<header><h1>Transcriber <b>panel</b></h1>
<span>localhost:7302 &middot; runs on this machine &middot; nothing uploaded</span></header>
...
def build_command(b):
    tool = os.path.join(HERE, "transcribe.py")
    f = b.get("file", "").strip()
    ...
    if os.path.abspath(f) != os.path.abspath(dst):
        with open(f, "rb") as a, open(dst, "wb") as c:
            c.write(a.read())
    cmd = [sys.executable, tool, dst, "--model", b.get("model", "tiny.en")]
    if b.get("speakers") == "yes":
        cmd.append("--speakers")
    return cmd, None
